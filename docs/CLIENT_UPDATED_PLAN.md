...
1. **Project identity** (how to name things in code & env)
2. **Overview → scope + non-functional requirements**
3. **Stage 1 – Client & Order Intake** with exact fields, rules, APIs, DB schema, Ashley checks, edge cases, notifications, and acceptance tests
...
## Project Foundation
- [🏷️ Project Identity](#-project-identity)
- [🎯 Overview (Developer Lens)](#-overview-developer-lens)
- [📚 Glossary & Conventions](#-glossary--conventions)

## Production Stages
- [🔹 Stage 1 – Client & Order Intake](#-stage-1--client--order-intake-full-developer-spec)
- [🔹 Stage 2 – Design & Approval](#-stage-2--design--approval-developer-spec)
- [🔹 Stage 3 – Cutting](#-stage-3--cutting-developer-spec)
- [🔹 Stage 4 – Printing](#-stage-4--printing-developer-spec-per-method)
- [🔹 Stage 5 – Sewing](#-stage-5--sewing-developer-spec)
- [🔹 Stage 6 – Quality Control (QC)](#-stage-6--quality-control-qc--developer-spec)
- [🔹 Stage 7 – Finishing & Packing](#-stage-7--finishing--packing--developer-spec)
- [🔹 Stage 8 – Delivery](#-stage-8--delivery-developer-spec)

## Business Functions
- [🔹 Stage 9 – Finance](#-stage-9--finance-developer-spec)
- [🔹 Stage 10 – HR](#-stage-10--hr-attendance-payroll-compliance--developer-spec)
- [🔹 Stage 11 – Maintenance](#-stage-11--maintenance-machines--vehicles--developer-spec)

## Client & AI Features
- [🔹 Stage 12 – Client Portal](#-stage-12--client-portal-tracking-approvals-payments-reorders)
- [🔹 Stage 13 – Merchandising AI](#-stage-13--merchandising-ai-reprint-advisor--theme-recommender)
- [🔹 Stage 14 – Automation & Reminders](#-stage-14--automation--reminders-jobs-alerts-messaging)
...
## Core Terms
- **PO (Production Order)**: Complete order from client intake to delivery
- **Workcenter**: Standardized production areas (CUTTING, PRINTING, SEWING, QC, PACKING, etc.)
- **Bundle**: Physical grouping of items during production (use bundle_id consistently)
- **Batch**: Inventory/material grouping (use batch_id for inventory_batches)
- **GA**: Graphic Artist
- **CSR**: Customer Service Representative
- **Ashley**: The AI assistant/validation system
- **RLS**: Row Level Security
- **BFF**: Backend for Frontend (API Gateway pattern)

## Code Conventions
- **Table Names**: Snake_case (e.g., `order_lines`, `routing_steps`)
- **API Endpoints**: Kebab-case (e.g., `/orders/{id}/routing/apply-template`)
- **Event Names**: Dot notation (e.g., `ash.po.created`)
- **Environment Variables**: ASH_ prefix (e.g., `ASH_DB_URL`)
...
### That’s the level of detail your dev needs for Stage 1.
 ...
 Perfect—let’s turn **Stage 2 (Design & Approval)** and **Stage 3 (Cutting)** into a **developer-grade spec** with fields, flows, DB schema, APIs, validations, Ashley checks, formulas, events, and acceptance tests.
...
* **Cutting outputs & bundles** (Stage 3) feed **Sewing**’s WIP and **Payroll** (piece-rate), and they anchor **traceability** through Printing/Sewing/QC.
 ...
 Awesome—continuing the deep-dive. Below is a **developer-grade spec** for:

* 🔹 **Stage 4 – Printing (by method)**: Silkscreen, Sublimation, DTF, Embroidery
* 🔹 **Stage 5 – Sewing** (including sub-assemblies and parallel routing)

I’m keeping structure consistent: **actors & permissions → UX/flows → data model → APIs → validations & Ashley checks → calculations → events & edge cases → acceptance tests**.
(Names stay within **ASH AI** conventions.)
...
- Stage 3: Cutting (basic functionality)
- Stage 9: Finance (core invoicing)
- Core infrastructure and security framework

### Phase 2 - Production Core (Months 4-6)
- Stage 2: Design & Approval
- Stage 4: Printing (all methods)
- Stage 5: Sewing
...
- Advanced Ashley AI features

## Change Log

### Version 2.1 (Current)
- Added comprehensive Stage 10 HR specifications
- Enhanced Ashley AI validation rules across all stages
- Improved API consistency and error handling patterns
- Added detailed acceptance criteria for all stages

### Version 2.0
- Complete rewrite with developer-grade specifications
- Added comprehensive data models for all stages
- Standardized API patterns across all endpoints
